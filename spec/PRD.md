...
## 1.1 产品背景
小团队或小型仓库常用 Excel / 纸张记录货品进出库，效率低且容易出错。本产品提供一个 **基于扫码枪 + 云表格服务（AITable/Notion 等）的通用客户端**，帮助仓库管理员快速进行货品入库、出库与盘点。
...
- 本地缓存货品列表（SQLite / IndexedDB）
- 全部查询操作在本地完成
- 若货品状态更新失败（网络异常）→ 保存在本地“待同步队列”
- 网络恢复时自动同步
