...
## 1.1 产品背景
小团队或小型仓库常用 Excel / 纸张记录货物进出库，效率低且容易出错。本产品提供一个 **基于扫码枪 + 云表格服务（AITable/Notion 等）的通用客户端**，帮助仓库管理员快速进行货物入库、出库与盘点。

无需复杂配置，支持自动读取云表格的字段结构，适配任何表格格式。通过 Provider 架构设计，可轻松扩展支持多个云服务提供者。

# 2. 产品目标

- ✅ 通过扫码快速完成入库、出库、盘点  
- ✅ 无需账号系统，使用者仅需填写工号 + 云服务 API Key  
- ✅ 自动读取云表格结构，客户端动态生成字段表单  
- ✅ 支持扫码多个货物一次性出库  
- ✅ 基础离线能力：本地缓存货物列表  
- ✅ Windows 优先，采用 Electron 客户端，全屏扫码模式
- ✅ **支持多云服务提供者**（当前支持 AITable，可扩展 Notion、Google Sheets 等）  

# 3. 使用角色

| 角色 | 权限 |
|------|------|
| 仓库管理员 | 可进行入库、出库、盘点；可设置 API Key；可开启全屏模式；可选择云服务提供者 |

无账号系统，仅启动时填写：  
- **云服务提供者**（AITable / Notion / ...）
- 工号  
- 云服务 API Key  
- 数据表 ID / 视图 ID（外部配置）
...
## ✅ 4.1 动态表结构加载
- 客户端启动后，通过云服务 Provider API 获取：
  - 表字段结构
  - 指定视图内的货物列表
- 客户端根据字段动态构建 UI
- 字段类型自动识别（文本、数字、选择、日期、附件等）
- **支持多个云服务提供者**，通过 ProviderFactory 统一管理
...
- 本地缓存货物列表（SQLite / IndexedDB）
- 全部查询操作在本地完成
- 若货物状态更新失败（网络异常）→ 保存在本地“待同步队列”
- 网络恢复时自动同步
